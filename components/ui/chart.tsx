...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  // テーマ名のサニタイゼーション
  const sanitizeThemeName = (theme: string): string => {
    return theme.replace(/[^a-zA-Z0-9-_]/g, "");
  };
  ...
  const generateSafeCSS = () => {
    return Object.entries(THEMES)
      .map(([theme, prefix]) => {
        const safeTheme = sanitizeThemeName(theme);
        const cssRules = colorConfig
          .map(([key, itemConfig]) => {
            ...
            if (color) {
              ...
              const safeKey = sanitizeThemeName(key);
